...
import './DigitalDisplayStatusStyles.css';
...
import { DigitalDisplayDetail } from '../DigitalDisplayDetail';
import { DigitalDisplayEdit } from '../DigitalDisplayEdit/DigitalDisplayEdit.js';
...
export const DigitalDisplayStatus = () => {
  const { get } = useAxios();
  ...
  const trueItems = useTable({ endpoint: '/digital-display/details', enter: true});
  const falseItems = useTable({ endpoint: '/digital-display/available', enter:true });
  const toggleUpdate = () =>  {
    trueItems.search();
    falseItems.search();
  }
  ...
  const redirectList = () => history.push('/digital-display/list');

  const [showDeleteModal, setShowDeleteModal] = useState(false);

  const onClickCardHadler = (i) => {
    setShowDetailModal(true);
    setSelectedDigitalDisplay(i.id);
  };
  ...
  const tabsLabels =
    role === 4 || role === 5
      ...
      : ['Displays asignados', 'Displays disponibles'];
    ...
    <div>
      ...
      <div className="row mt-4">
        ...
        <div className="col-md-2">
          <Button onClick={redirectList}>Gestionar displays</Button>
        </div>
      </div>
      <NetscaPagination
