...
import { getDigitalDisplay, editDigitalDisplay, resetProductsDigitalDisplay ,getClientList, getProductList, getBranchList} from 'providers/api';
import { messages as formMessages } from './DigitalDisplayEditMessages';
...
export const useDigitalDisplayEdit = ({ displayId, onAccept, toggleUpdate }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        await editDigitalDisplay(displayId, data);
        toggleUpdate();
        onAccept();
      } catch (error) {
  ...
  useEffect(async () => {
    if(formik.values.client_id > 0){      
      let branches = await  getBranchList(formik.values.client_id);
  ...
  useEffect(async () => {
    if(formik.values.branch_id > 0){
      let productsResponse = await  getProductList(formik.values.branch_id,searchProduct);
  ...
  useEffect(async () => {
    let data = await getDigitalDisplayData();
    
    if(data.client_id && data.branch_id){
  ...
  async function getDigitalDisplayData() {
    let displayData = initialDisplayValues;

    if (displayId != 0) {
      try {
        const getDisplayResponse = await getDigitalDisplay(displayId);
...
const onChangeDigitalProducts = (productSelected, row, col, pageSelected) => {
  ...
  const LoadAllProducts = async () => {
    let productsResponse = await  getProductList(formik.values.branch_id, '');
