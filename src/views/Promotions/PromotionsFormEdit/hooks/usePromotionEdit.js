...
const validationSchemas = yup.object().shape({
  ...
  validity_type: yup.string().required(requiredMsg),
  ...
  sale_off_type_id: yup.string().required(requiredMsg),
  historicPoints: yup.boolean(),
  purchasedProducts: yup.boolean(),
  code: yup.boolean(),
  ...
  require_options: yup.string().required(requiredMsg),
  code_text: yup.string().when('code', {
    is: true,
    then: yup
      .string()
      .required(requiredMsg)
      .matches(
        /^\s*-?[0-9]{1,51}\s*$/,
        intlExt.formatMessage(formMessages.barcode_boxLength)
      )
      .max(50, intlExt.formatMessage(formMessages.number_10_length))
  }),
  limit: yup.string().required(requiredMsg),
  limit_quantity: yup.string().when('limit', {
    is: 'exactly',
    then: yup
      .string()
      .required(requiredMsg)
...
var initialObj = {
  ...
  require_options: yup.string().required(requiredMsg),
  client_id_0: yup.string().required(requiredMsg),
  // branches_id_0:  yup
  //   .string()
  //   .required(requiredMsg),
  branches_id_0: yup.array().min(1),
  code_text: yup.string().when('code', {
    is: true,
    then: yup
      .string()
  ...
  limit: yup.string().when('sale_off_type_id', {
    is: '1',
    then: yup.string().required(requiredMsg)
  }),
  limit_quantity: yup.string().when('limit', {
...
export const usePromotionEdit = (
  ...
  const [yupSchema, setYupSchema] = useState(yup.object().shape(yupObject));
  const [validationSchema, setValidationSchema] = useState(
    yup.object().shape(yupObject)
  );
  ...
  useEffect(() => {
    if (typePromotion === 'Cliente') {
      setValidationSchema(yup.object().shape(initialObjClient));
    } else {
      setValidationSchema(yup.object().shape(initialObj));
    }
    GetDivision();
    // GetProduct();
    GetClient();
  ...
  const addProductList = () => {
    ...
    //   products: [],
    // })
    schema[`product_id_${list.length}`] = yup
      .string()
      .when('purchasedProducts', {
        is: true,
        then: yup.string().required(requiredMsg)
      });
    schema[`unit_type_id_${list.length}`] = yup
      .string()
      .when('purchasedProducts', {
        is: true,
        then: yup.string().required(requiredMsg)
      });
    schema[`minimum_quantity_${list.length}`] = yup
      .string()
      .when('purchasedProducts', {
        is: true,
        then: yup
  ...
  const generateProductList = async (array, array_clients, promotion) => {
    ...
    for (let i = 0; i < array.length; i++) {
      let response_category = await doGet(
        `/sale-off/division/${array[i].product_division_id}/category/list`
      );
      let response_material = await doGet(
        `/sale-off/category/${array[i].product_category_id}/material-group/list`
      );
      let response_product = await doGet(
        `/sale-off/material-group/${array[i].product_material_group_id}/product/list`
      );
    ...
    array.map((element, index) => {
      ...
      schema[`minimum_quantity_${index}`] = yup
        .string()
        .when('purchasedProducts', {
          is: true,
          then: yup
            ...
            .matches(
              /^\d{1,4}/,
              intlExt.formatMessage(formMessages.onlyNumbers)
            )
            .max(3, intlExt.formatMessage(formMessages.pointsLength))
        });
  ...
  const generateClientList = async (array_clients, promotion) => {
    ...
    for (let i = 0; i < array_clients.length; i++) {
      let response_branches = await doGet(
        `/sale-off/branch/list/${array_clients[i].client_id}`
      );
    ...
    array_clients.map((element, index) => {
      ...
      if (element.branch_ids.length >= branches_list[index].length) {
        selected = true;
      } else {
        selected = false;
      }
      ...
      list.push({
        ...
        branch_ids: element.branch_ids,
        branch_names: element.branch_names,
        visible: true,
        deleted: false,
        clients: client_list,
  ...
  const handleSave = (endpoint, data, typePromotion) => {
    ...
    if (data.validity_date && data.validity_type === 'active_until')
      fd.set('validity_date', data.validity_date);
    ...
    if (data.historicPoints) {
      ...
      if (data.quantity && typePromotion === 1)
        fd.set('quantity', data.quantity);
    }
    if (typePromotion === 1) fd.set('load_points', data.purchasedProducts);
    if (data.purchasedProducts) {
      if (data.radio_button_purchasedProducts && typePromotion === 1)
        fd.set('product_compliance', data.radio_button_purchasedProducts);
    ...
    if (data.code && data.code_text && typePromotion === 1)
      fd.set('code', data.code_text);
    if (typePromotion === 1) fd.set('limit', data.limit);
    if (data.limit_quantity && typePromotion === 1)
      fd.set('limit_quantity', data.limit_quantity);
    ...
    if (typePromotion === 3) {
      if (listClients && data.branches_id_0) {
        generateClientArray();
      }
  ...
  const GetProduct = async (valmaterial, index) => {
    ...
    await axios
      ...
      .then((response) => {
        if (response.status === 200) {
          let list = listProducts.slice();
          (list[index].products = response.data.items), setListProducts(list);
    ...
    await axios
      ...
      .then((response) => {
        if (response.status === 200) {
          let list = listClients.slice();
          list.forEach((element) => {
            element.branches = response.data.items;
          });
  ...
  const GetBranchId = async (id, index) => {
    ...
    await axios
      ...
      .then((response) => {
        if (response.status === 200) {
          let list = listClients.slice();
          (list[index].branches = response.data.items), setListClients(list);
        }
      })
      .finally(() => {
        // updateLoadingCampaigns(false);
  ...
  const PromoValues = async (promo) => {
    ...
    if (values.sale_off_type_name === 'Cliente') {
      if (values.clients.length > 0) {
        let list = [];
        let client_list = await doGet(`/sale-off/client/list`);
        let branches_list = await doGet(`/sale-off/branch/list/`);
        let selected = false;

        if (values.clients[0].branch_ids.length >= branches_list.length) {
          selected = true;
        } else {
          selected = false;
        }
        ...
        list.push({
          client_id: values.clients[0].client_id,
          branch_ids: values.clients[0].branch_ids,
          branch_names: values.clients[0].branch_names,
          visible: true,
          deleted: false,
          clients: client_list,
        ...
        setListClients(list);
        formikFunction('branches_id_0', values.clients[0].branch_ids);
      }
    }
    ...
    if (values.image_url) {
      ...
      formikFunction(
        'user_image',
        config.siteConfig.apiUrl + '/' + values.image_url
      );
      formikFunction('user_image_size', 1000);
      formikFunction('user_image_type', 'image/png');
    }
    if (values.limit) {
    ...
    if (values.value) {
      formikFunction('value', values.value);
    }
  };
