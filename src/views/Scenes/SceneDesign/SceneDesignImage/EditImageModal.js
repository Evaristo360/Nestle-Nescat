...
import { editImageMessages as formMessages } from '../SceneDesignMessages';
...
const EditImageModal = ({
  ...
  const saveData = () => {
    if (validateForm()) {
      ...
      doPatch(`/advertisements/image/design/${state.id}`, data).then(
        ({ result }) => {
          onAccept({ ...state, media_type: 'image', isEdit: true });
        }
      );
    }
  };
  ...
  return (
    <Modal visible onClose={onClose} background={currentTheme.background}>
      <div className="container" css={styles}>
        ...
        <label className="text mt-3" style={{ fontSize: '15px' }}>
          {getMsg('subtitle')}
        </label>
